...
{{-- HERO --}}
<div class="sd-hero">
    <div class="sd-av">{{ $initials }}</div>
    <div class="sd-ht">
        <h1>Halo, {{ Str::words($data['nama_santri'], 2) }} &#128075;</h1>
        <p>
            Kelas {{ $data['kelas'] }}
            @if($semesterAktif)
            <span class="sd-sem"><i class="fas fa-calendar-check"></i> {{ $semesterAktif->nama_semester }}</span>
            @endif
        </p>
    </div>
    <div class="sd-hd">
        <strong>{{ now()->locale('id')->isoFormat('dddd') }}</strong>
...
<div class="sd-alert red">
    <span>&#127973;</span>
    <div>
        <strong>Sedang dalam perawatan UKP</strong> sejak {{ $statusKesehatan->tanggal_masuk_formatted }}
        ({{ $statusKesehatan->lama_dirawat }} hari). Keluhan: <em>{{ $statusKesehatan->keluhan }}</em>.
        <a href="{{ route('santri.kesehatan.index') }}">Lihat detail &rarr;</a>
    </div>
</div>
@endif
...
<div class="sd-alert blue">
    ...
    <div>
        ...
        ({{ $kepulanganAktif->tanggal_pulang_formatted }} &ndash; {{ $kepulanganAktif->tanggal_kembali_formatted }}).
        Pastikan kembali tepat waktu!
        <a href="{{ route('santri.kepulangan.show', $kepulanganAktif->id_kepulangan) }}">Lihat detail &rarr;</a>
    </div>
</div>
@endif
...
<div class="sd-kpi-g">
    <div class="sd-kpi kv-g">
        ...
        <div class="sd-kpi-val" style="font-size:1.1rem">Rp {{ number_format($data['saldo_uang_saku'],0,',','.') }}</div>
        <div class="sd-kpi-lbl">Saldo Uang Saku</div>
        <div class="sd-kpi-ft"><a href="{{ route('santri.uang-saku.index') }}" class="sd-a g">Riwayat <i class="fas fa-arrow-right" style="font-size:.58rem"></i></a></div>
    </div>
    <div class="sd-kpi {{ $poin==0?'kv-g':($poin<50?'kv-a':'kv-r') }}">
        <div class="sd-kpi-top"></div>
        <div class="sd-kpi-ico"><i class="fas fa-shield-alt"></i></div>
        <div class="sd-kpi-val">{{ $poin }}</div>
        <div class="sd-kpi-lbl">Poin Pelanggaran</div>
        <div class="sd-kpi-ft">
            @if($poin==0)<span style="color:#059862;font-weight:700;font-size:.74rem">&#10003; Bersih</span>
            @else<a href="{{ route('santri.pelanggaran.index') }}" class="sd-a">Lihat <i class="fas fa-arrow-right" style="font-size:.58rem"></i></a>@endif
        </div>
    </div>
    <div class="sd-kpi kv-b">
        <div class="sd-kpi-top"></div>
        <div class="sd-kpi-ico"><i class="fas fa-graduation-cap"></i></div>
        <div class="sd-kpi-val">{{ $avgCap }}%</div>
        <div class="sd-kpi-lbl">Rata-rata Capaian Belajar</div>
        <div class="sd-kpi-ft"><a href="{{ route('santri.capaian.index') }}" class="sd-a">Detail <i class="fas fa-arrow-right" style="font-size:.58rem"></i></a></div>
    </div>
    <div class="sd-kpi kv-t">
        <div class="sd-kpi-top"></div>
        <div class="sd-kpi-ico"><i class="fas fa-calendar-day"></i></div>
        <div class="sd-kpi-val">{{ now()->locale('id')->isoFormat('dddd') }}</div>
        <div class="sd-kpi-lbl">Jadwal Kegiatan Aktif</div>
        <div class="sd-kpi-ft"><a href="{{ route('santri.kegiatan.index') }}" class="sd-a t">Cek Jadwal <i class="fas fa-arrow-right" style="font-size:.58rem"></i></a></div>
    </div>
</div>

{{-- KEPULANGAN --}}
<div class="sd-sec"><i class="fas fa-door-open"></i> Kepulangan Tahun {{ now()->year }}</div>
<div class="sd-card">
    <div class="sd-ch">
        <h3><i class="fas fa-calendar-minus" style="color:var(--teal)"></i> Kuota Izin Kepulangan</h3>
        <a href="{{ route('santri.kepulangan.index') }}" class="sd-a"><i class="fas fa-list"></i> Lihat Semua</a>
    </div>
    <div style="padding:18px 20px">
        <div class="sd-kq-nums">
            <div class="sd-kq-num">
                <div class="v" style="color:var(--ink)">{{ $kpTotalHari }} <span style="font-size:.74rem;font-weight:600;color:var(--ink3)">/ 12 hari</span></div>
                <div class="s">Total hari terpakai</div>
            </div>
            <div class="sd-kq-num">
                <div class="v" style="color:{{ $kpOver?'var(--red)':'var(--green)' }}">{{ $kpSisa }} hari</div>
                <div class="s">Sisa kuota</div>
            </div>
            <div class="sd-kq-num">
                <div class="v" style="color:var(--blue)">{{ $kpDisetujui }}</div>
                <div class="s">Kali disetujui</div>
            </div>
            @if($kpMenunggu > 0)
            <div class="sd-kq-num" style="align-self:center">
                <span class="sd-pill p-a"><i class="fas fa-clock"></i> {{ $kpMenunggu }} menunggu approval</span>
            </div>
            @endif
        </div>
        <div class="sd-qbar">
            <div class="sd-qfill {{ $kpOver?'o':($kpPersen>=75?'w':'') }}" style="width:{{ min(100,$kpPersen) }}%"></div>
        </div>
        <div class="sd-qmeta">
            <span>{{ $kpTotalHari }} dari 12 hari kuota</span>
            @if($kpOver)<span style="color:var(--red);font-weight:700">&#9888; Melebihi kuota!</span>
            @else<span>Sisa {{ $kpSisa }} hari</span>@endif
        </div>
    </div>
</div>
...
<div class="sd-2">
    ...
    <div class="sd-card">
        <div class="sd-ch">
            ...
            <a href="{{ route('santri.capaian.index') }}" class="sd-a p">Buka Halaman</a>
        </div>
        <div class="sd-ci-top">
            <div style="padding-top:1px">
                <div class="sd-ci-badge {{ $ciBadge }}">
                    @if($ciIsOpen && !$ciSegera)<div class="ci-pulse"></div>@endif
                    {{ $ciBLabel }}
                </div>
            </div>
            <div style="flex:1">
                @if($semesterAktif)
                <div style="font-size:.86rem;font-weight:700;color:var(--ink);margin-bottom:3px">{{ $semesterAktif->nama_semester }}</div>
                @endif
                @if($ciIsOpen && $ciDeadline)
                <div style="font-size:.75rem;color:var(--ink3);line-height:1.5">
                    <i class="fas fa-clock" style="font-size:.63rem;margin-right:3px"></i>
                    Deadline: <strong style="color:{{ $ciSegera?'var(--amber)':'var(--ink2)' }}">
                        {{ \Carbon\Carbon::parse($ciDeadline)->locale('id')->isoFormat('D MMMM YYYY') }}
                    </strong>
                    @if($ciSegera)
                    &bull; <span style="color:var(--amber);font-weight:700">{{ \Carbon\Carbon::parse($ciDeadline)->diffForHumans() }}</span>
                    @endif
                </div>
                @elseif(!$ciIsOpen)
        ...
        <div class="sd-ci-prog">
            <div style="display:flex;justify-content:space-between;font-size:.78rem">
                ...
                <span style="font-weight:800;color:var(--blue)">{{ $ciSudah }} / {{ $ciTotal }}</span>
            </div>
            <div class="sd-ci-bar"><div class="sd-ci-fill" style="width:{{ $ciPersen }}%"></div></div>
            <div style="font-size:.69rem;color:var(--ink3);text-align:right">{{ $ciPersen }}% sudah diisi</div>
        </div>
        @else
        <div class="sd-ci-prog">
        ...
        <div class="sd-ci-cats">
            ...
            <div class="sd-ci-cat">
                <div class="sd-ci-cat-name">
                    <div class="sd-ci-dot" style="background:{{ $kColor }}"></div>
                    {{ $kNama }}
                </div>
                <div class="sd-ci-cat-bar">
                    <div class="sd-ci-cat-fill" style="width:{{ $kPct }}%;background:{{ $kColor }}"></div>
                </div>
                <div class="sd-ci-cat-pct" style="color:{{ $kColor }}">{{ $kPct }}%</div>
            </div>
            @endforeach
        </div>
    </div>
    ...
    <div class="sd-card">
        <div class="sd-ch">
            ...
            <a href="{{ route('santri.berita.index') }}" class="sd-a">Semua</a>
        </div>
        {{-- limit & filter dikontrol controller: latest 5, tanpa filter tanggal --}}
        @forelse($beritaTerbaru as $berita)
        <a href="{{ route('santri.berita.show', $berita->id_berita) }}" class="sd-bitem">
            <div class="sd-bdot"></div>
            <div>
                <div class="sd-btit">{{ $berita->judul }}</div>
                <div class="sd-bmeta"><i class="fas fa-clock" style="font-size:.61rem"></i> {{ $berita->created_at->diffForHumans() }}</div>
            </div>
        </a>
        @empty
...
<div class="sd-card" style="margin-bottom:13px">
    <div class="sd-ch">
        ...
        <div style="display:flex;align-items:center;gap:8px">
            <div class="sd-toggle">
                <button class="sd-toggle-btn active" id="btnBulan" onclick="switchPeriode('bulan')">{{ $bulanNama }}</button>
                <button class="sd-toggle-btn" id="btnMinggu" onclick="switchPeriode('minggu')">{{ $mingguLabel }}</button>
            </div>
            <a href="{{ route('santri.kegiatan.index') }}" class="sd-a t">Detail</a>
        </div>
    </div>
...
<div class="sd-card">
    <div class="sd-ch">
        ...
        <div style="display:flex;align-items:center;gap:8px">
            <span class="sd-pill p-r">{{ $poin }} poin total</span>
            <a href="{{ route('santri.pelanggaran.index') }}" class="sd-a">Semua</a>
        </div>
    </div>
    <table class="sd-tbl">
        <thead><tr><th>Tanggal</th><th>Pelanggaran</th><th>Poin</th></tr></thead>
        <tbody>
            @forelse($pelanggaranTerbaru ?? [] as $pel)
            <tr>
                <td style="color:var(--ink2);white-space:nowrap">{{ \Carbon\Carbon::parse($pel->tanggal)->locale('id')->isoFormat('D MMM YY') }}</td>
                <td>{{ $pel->kategori->nama_pelanggaran ?? ($pel->keterangan ?? '-') }}</td>
                <td><span class="sd-pill p-r">{{ $pel->poin }}</span></td>
            </tr>
            @empty
...
const DATA = {
    ...
    minggu: {
        labels: @json($barKatMgg),
        hadir:  @json($barHadirMgg),
        alpa:   @json($barAlpaMgg),
        izin:   @json($barIzinMgg),
        sakit:  @json($barSakitMgg),
    }
};
