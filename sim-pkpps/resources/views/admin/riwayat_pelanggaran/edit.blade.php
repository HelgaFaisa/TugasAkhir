{{-- resources/views/admin/riwayat_pelanggaran/edit.blade.php --}}
@extends('layouts.app')
...
<div style="margin-bottom: 20px;">
    <nav style="display: flex; align-items: center; gap: 8px; color: var(--text-light); font-size: 0.9em;">
        ...
        <span>Edit {{ $riwayatPelanggaran->id_riwayat }}</span>
    </nav>
</div>
...
<div class="content-box">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 25px;">
        <h3 style="margin: 0; color: var(--primary-color);">
            <i class="fas fa-edit"></i> Form Edit Riwayat
        </h3>
        <div style="background: var(--primary-light); padding: 10px 20px; border-radius: var(--border-radius-sm);">
            <small style="color: var(--text-light);">ID Riwayat:</small>
            <strong style="color: var(--primary-dark); font-size: 1.1em;">{{ $riwayatPelanggaran->id_riwayat }}</strong>
        </div>
    </div>
    ...
    <form action="{{ route('admin.riwayat-pelanggaran.update', $riwayatPelanggaran) }}" method="POST">
        ...
        <div class="form-group">
            ...
            <textarea name="keterangan" 
                      ...
                      placeholder="Jelaskan detail pelanggaran (opsional)">{{ old('keterangan', $riwayatPelanggaran->keterangan) }}</textarea>
            @error('keterangan')
                <span class="invalid-feedback">{{ $message }}</span>
            @enderror
...
// Trigger change event untuk menampilkan poin saat halaman load
