...
use App\Http\Controllers\Santri\SantriCapaianInputController;  // <-- BARU
...
// PERBAIKAN: tambah ->name('admin.') agar tidak cross-redirect dengan santri
...
// PERBAIKAN: pisah prefix name agar tidak tabrakan dengan santri authenticated group
...
Route::prefix('admin')->name('admin.')->group(function () {
    ...
    Route::middleware(['auth', 'role:super_admin'])->group(function () {
        ...
        Route::prefix('users')->name('users.')->group(function () {
            ...
            Route::get('admin', [UserController::class, 'adminAccounts'])
                ->name('admin_accounts');
            Route::get('admin/create', [UserController::class, 'createAdminAccount'])
                ->name('admin_create');
            Route::post('admin/store', [UserController::class, 'storeAdminAccount'])
                ->name('admin_store');
            Route::get('admin/{userId}/edit', [UserController::class, 'editAdminAccount'])
                ->name('admin_edit');
            Route::put('admin/{userId}', [UserController::class, 'updateAdminAccount'])
                ->name('admin_update');
            Route::delete('admin/{userId}/hapus', [UserController::class, 'destroyAdminAccount'])
                ->name('admin_destroy');
        });
        ...
        Route::prefix('pembayaran-spp')->name('pembayaran-spp.')->group(function () {
            ...
            Route::get('/cetak-laporan', [PembayaranSppController::class, 'cetakLaporan'])
                ->name('cetak-laporan');
            Route::get('/cetak-laporan-santri/{id_santri}', [PembayaranSppController::class, 'cetakLaporanSantri'])
                ->name('cetak-laporan-santri');
            Route::get('/{pembayaranSpp}/cetak-bukti', [PembayaranSppController::class, 'cetakBukti'])
                ->name('cetak-bukti');
            Route::get('/riwayat/{id_santri}', [PembayaranSppController::class, 'riwayat'])
                ->name('riwayat');
    ...
    Route::middleware(['auth', 'role:super_admin,akademik'])->group(function () {
        ...
        // -- Rekap Absensi --
        Route::get('absensi-kegiatan/rekap/{kegiatan_id}', [AbsensiKegiatanController::class, 'rekapAbsensi'])
        ...
        Route::prefix('riwayat-kegiatan')->name('riwayat-kegiatan.')->group(function () {
            ...
            Route::get('/detail-santri/{id_santri}', [RiwayatKegiatanController::class, 'detailSantri'])
                ->name('detail-santri');
        ...
        Route::prefix('laporan-kegiatan')->name('laporan-kegiatan.')->group(function () {
            ...
            Route::get('/detail-santri/{id_santri}', [LaporanKegiatanController::class, 'detailSantri'])
                ->name('detail-santri');
            Route::get('/santri-perlu-perhatian', [LaporanKegiatanController::class, 'santriPerluPerhatian'])
                ->name('santri-perlu-perhatian');
            Route::get('/leaderboard', [LaporanKegiatanController::class, 'leaderboard'])
                ->name('leaderboard');
            Route::get('/analisis-kegiatan/{kegiatan_id}', [LaporanKegiatanController::class, 'analisKegiatan'])
                ->name('analisis-kegiatan');
            Route::get('/analisis-kelas', [LaporanKegiatanController::class, 'analisPerKelas'])
                ->name('analisis-kelas');
            Route::get('/patterns', [LaporanKegiatanController::class, 'patternDetection'])
                ->name('patterns');
            Route::get('/export-excel', [LaporanKegiatanController::class, 'exportExcel'])
                ->name('export-excel');
            Route::get('/export-pdf', [LaporanKegiatanController::class, 'exportPdf'])
                ->name('export-pdf');
            Route::get('/refresh-kpi', [LaporanKegiatanController::class, 'refreshKpi'])
                ->name('refresh-kpi');
        });
    ...
    Route::middleware(['auth', 'role:super_admin,akademik,pamong'])->group(function () {
        ...
        Route::prefix('kegiatan')->name('kegiatan.')->group(function () {
            ...
            Route::get('/{kegiatan_id}/detail', [KegiatanController::class, 'getDetailModal'])
                ->name('detail-modal');
        ...
        Route::prefix('absensi-kegiatan')->name('absensi-kegiatan.')->group(function () {
            ...
            Route::get('/input/{kegiatan_id}', [AbsensiKegiatanController::class, 'inputAbsensi'])
                ->name('input');
        ...
        Route::prefix('capaian')->name('capaian.')->group(function () {
            ...
            Route::get('/detail-materi/{id_materi}', [CapaianController::class, 'detailMateri'])
                ->name('detail-materi');
            Route::post('/tandai-khatam/{id_santri}', [CapaianController::class, 'tandaiKhatam'])
                ->name('tandai-khatam');
            Route::post('/batal-khatam/{id_santri}', [CapaianController::class, 'batalKhatam'])
                ->name('batal-khatam');
            Route::get('/export-rapor/{id_santri}/{id_semester}', [CapaianController::class, 'exportRapor'])
                ->name('export-rapor');

            // ---- TAMBAHAN BARU: Kelola Akses Input Capaian oleh Santri ----
            // PENTING: taruh sebelum route /{capaian} agar tidak bentrok
            Route::get('/akses-santri', [CapaianController::class, 'kelolaAksesSantri'])
                ->name('akses-santri');
            Route::post('/akses-santri/buka', [CapaianController::class, 'bukaAksesSantri'])
                ->name('akses-santri.buka');
            Route::post('/akses-santri/tutup', [CapaianController::class, 'tutupAksesSantri'])
                ->name('akses-santri.tutup');
            // ----------------------------------------------------------------
            ...
            Route::get('/riwayat/{id_santri}', [CapaianController::class, 'riwayatSantri'])
                ->name('riwayat-santri');
            ...
            Route::post('/ajax/get-materi', [CapaianController::class, 'getMateriByKelas'])
                ->name('ajax.get-materi');
            Route::post('/ajax/get-detail-materi', [CapaianController::class, 'getDetailMateri'])
                ->name('ajax.get-detail-materi');
            Route::post('/ajax/calculate-persentase', [CapaianController::class, 'calculatePersentase'])
                ->name('ajax.calculate-persentase');
            Route::get('/api/grafik-data', [CapaianController::class, 'apiGrafikData'])
                ->name('api.grafik-data');
        });

        // -- Kesehatan Santri --
        Route::resource('kesehatan-santri', KesehatanSantriController::class);
        Route::patch('kesehatan-santri/{kesehatanSantri}/keluar-ukp', [KesehatanSantriController::class, 'keluarUkp'])
            ->name('kesehatan-santri.keluar-ukp');
        Route::get('kesehatan-santri/riwayat/{id_santri}', [KesehatanSantriController::class, 'riwayat'])
            ->name('kesehatan-santri.riwayat');
        Route::get('kesehatan-santri/{kesehatanSantri}/cetak-surat', [KesehatanSantriController::class, 'cetakSurat'])
        ...
        Route::prefix('kepulangan')->name('kepulangan.')->group(function () {
            ...
            Route::post('/pengajuan/{id}/approve', [KepulanganController::class, 'approvePengajuan'])
                ->name('pengajuan.approve');
            Route::post('/pengajuan/{id}/reject', [KepulanganController::class, 'rejectPengajuan'])
                ->name('pengajuan.reject');
            Route::get('/settings/manage', [KepulanganController::class, 'settings'])->name('settings');
            Route::put('/settings/update', [KepulanganController::class, 'updateSettings'])
                ->name('settings.update');
            Route::get('/over-limit/list', [KepulanganController::class, 'santriOverLimit'])
                ->name('over-limit');
            Route::get('/api/santri/{id_santri}', [KepulanganController::class, 'getSantriData'])
                ->name('api.santri');
            Route::post('/reset/santri/{id_santri}', [KepulanganController::class, 'resetKuotaSantri'])
                ->name('reset.santri');
            Route::post('/reset/semua', [KepulanganController::class, 'resetKuotaSemuaSantri'])
                ->name('reset.semua');
            ...
            Route::post('/{id_kepulangan}/approve', [KepulanganController::class, 'approve'])
                ->name('approve');
            Route::post('/{id_kepulangan}/reject', [KepulanganController::class, 'reject'])
                ->name('reject');
            Route::post('/{id_kepulangan}/complete', [KepulanganController::class, 'complete'])
                ->name('complete');
            Route::get('/{id_kepulangan}/print', [KepulanganController::class, 'print'])
                ->name('print');
        });
    });
    ...
    Route::middleware(['auth', 'role:super_admin,pamong'])->group(function () {
        ...
        Route::prefix('uang-saku')->name('uang-saku.')->group(function () {
            ...
            Route::get('/santri-info/{id_santri}', [UangSakuController::class, 'santriInfo'])
                ->name('santri-info');
            Route::get('/riwayat/{id_santri}', [UangSakuController::class, 'riwayat'])
                ->name('riwayat');
...
Route::prefix('santri')
    ...
    Route::prefix('pelanggaran')->name('pelanggaran.')->group(function () {
        ...
        Route::get('/kategori/daftar', [SantriPelanggaranController::class, 'kategoriList'])
            ->name('kategori');
        Route::get('/{riwayatPelanggaran}', [SantriPelanggaranController::class, 'show'])
            ->name('show');
    });
    ...
    Route::prefix('capaian')->name('capaian.')->group(function () {
        ...
        // ---- TAMBAHAN BARU: Input Capaian Mandiri oleh Santri ----
        // PENTING: semua route statis harus di atas /{id} agar tidak bentrok
        Route::get('/input', [SantriCapaianInputController::class, 'create'])
            ->name('input.create');
        Route::post('/input', [SantriCapaianInputController::class, 'store'])
            ->name('input.store');
        Route::post('/input/ajax/detail', [SantriCapaianInputController::class, 'ajaxDetailMateri'])
            ->name('input.ajax.detail');
        Route::post('/input/ajax/hitung', [SantriCapaianInputController::class, 'ajaxHitungPersentase'])
            ->name('input.ajax.hitung');
        // ----------------------------------------------------------

        Route::get('/api/grafik-data', [SantriCapaianController::class, 'apiGrafikData'])
            ->name('api.grafik-data');
    ...
    Route::prefix('kepulangan')->name('kepulangan.')->group(function () {
        ...
        Route::get('/{kepulangan:id_kepulangan}', [SantriKepulanganController::class, 'show'])
            ->name('show');
    });
